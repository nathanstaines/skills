...
Once done, review the full set of changes with `/code-review` before handing the work back. Close the task per the tracker conventions, only when every acceptance criterion is met and with any deviation from the task description noted. Implement only the referenced task: when it's done, stop rather than pulling the next frontier task into the same session. A fresh session can run `/progress` to pick the next frontier task up.
