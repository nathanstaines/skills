...
2. **Mark it in-progress** per the tracker conventions. Already in-progress means a session probably died mid-flight: say when it was last touched, then ask whether to continue from where it stands or reset it to its unstarted state and start clean. Picking one up blind is how half-finished work gets built on twice.
...
5. **Close the task** per the tracker conventions, only when every acceptance criterion is met and ticked and with any deviation from the task description noted. Then stop, and say the changes are unreviewed.
