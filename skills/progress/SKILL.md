...
**Local markdown**: read every `.scratchpad/*/spec.md` and every `.scratchpad/*/tasks/*.md`. Take the frontmatter block and the title line from each and stop there. The bodies are long and nothing in this report needs them, so reading them only burns the context the user wants left for the work itself.

**GitHub**: `gh issue list --state all --json number,title,state,labels,body`, grouping tasks under the `Spec:` issue each one references.
...
- `in-progress` with an `updated` more than a few days old
